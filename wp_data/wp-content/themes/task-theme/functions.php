...
// Define timber directories
if ( class_exists( 'Timber' ) ) Timber::$dirname = 'template-parts/partials';

if ( ! function_exists( 'task_theme_setup' ) ) :
	/**
	...
	function task_theme_setup() {
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'task-theme' ),
			)
		);
...
function task_theme_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'task-theme' ),
			'id'            => 'sidebar-1',
...
function task_theme_scripts() {
	wp_enqueue_style( 'task-theme-style', get_stylesheet_uri(), array(), _S_VERSION );
